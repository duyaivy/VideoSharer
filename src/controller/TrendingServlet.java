...
public class TrendingServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ArrayList<Video> videos = videoBO.getInstance().getVideoTrending(page, size);

			// Tính totalPages
			int totalVideos = videoBO.getInstance().countTrendingVideos();
			int totalPages = (int) Math.ceil((double) totalVideos / size);

			request.setAttribute("videos", videos);
			request.setAttribute("currentPage", page);
			request.setAttribute("totalPages", totalPages);
			request.setAttribute("isTrendingMode", true);
