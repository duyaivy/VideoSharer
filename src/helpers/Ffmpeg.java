...
public class Ffmpeg {
    ...
    // ✅ HARDCODE PATH ĐỂ TEST
    private static final String HARDCODED_FFMPEG_DIR = "D:\\ECLIPSE2020\\VideoSharer\\WebContent\\WEB-INF\\lib";
    
    private static final int MAXIMUM_BITRATE_720P = 5 * 1000000; 
    private static final int MAXIMUM_BITRATE_1080P = 8 * 1000000; 
    private static final int MAXIMUM_BITRATE_1440P = 16 * 1000000; 
    ...
    private static String getFfmpegPath() {
        ...
        // ✅ 1. Ưu tiên dùng hardcoded path
        File hardcodedFile = new File(HARDCODED_FFMPEG_DIR, "ffmpeg.exe");
        System.out.println("🔍 Checking hardcoded path: " + hardcodedFile.getAbsolutePath());
        
        if (hardcodedFile.exists() && hardcodedFile.isFile()) {
            FFMPEG_PATH = hardcodedFile.getAbsolutePath();
            System.out.println("✅ Found ffmpeg at (HARDCODED): " + FFMPEG_PATH);
            return FFMPEG_PATH;
        } else {
            System.err.println("❌ ffmpeg.exe NOT FOUND at hardcoded path: " + hardcodedFile.getAbsolutePath());
        }

        // ✅ 2. Fallback: System PATH
        FFMPEG_PATH = "ffmpeg";
        System.out.println("⚠️ Using system ffmpeg (must be in PATH)");
        return FFMPEG_PATH;
    }
    ...
    private static String getFfprobePath() {
        ...
        // ✅ 1. Ưu tiên dùng hardcoded path
        File hardcodedFile = new File(HARDCODED_FFMPEG_DIR, "ffprobe.exe");
        System.out.println("🔍 Checking hardcoded path: " + hardcodedFile.getAbsolutePath());
        
        if (hardcodedFile.exists() && hardcodedFile.isFile()) {
            FFPROBE_PATH = hardcodedFile.getAbsolutePath();
            System.out.println("✅ Found ffprobe at (HARDCODED): " + FFPROBE_PATH);
            return FFPROBE_PATH;
        } else {
            System.err.println("❌ ffprobe.exe NOT FOUND at hardcoded path: " + hardcodedFile.getAbsolutePath());
        }

        // ✅ 2. Fallback: System PATH
        FFPROBE_PATH = "ffprobe";
        System.out.println("⚠️ Using system ffprobe (must be in PATH)");
        return FFPROBE_PATH;
    }
    ...
    public static boolean checkVideoHasAudio(String filePath) {
        try {
            ...
            ProcessBuilder pb = new ProcessBuilder(ffprobeExe, "-v", "error", "-select_streams", "a:0", 
                    "-show_entries", "stream=codec_type", "-of", "default=nw=1:nk=1", filePath);
    ...
    public static int getBitrate(String filePath) {
        try {
            ...
            ProcessBuilder pb = new ProcessBuilder(ffprobeExe, "-v", "error", "-select_streams", "v:0", 
                    "-show_entries", "stream=bit_rate", "-of", "default=nw=1:nk=1", filePath);
    ...
    private static boolean encodeMax720(String inputPath, String outputPath, String outputSegmentPath,
            Resolution resolution, Bitrate bitrate, boolean isHasAudio) {
        try {
    ...
    private static boolean encodeMax1080(String inputPath, String outputPath, String outputSegmentPath,
            Resolution resolution, Bitrate bitrate, boolean isHasAudio) {
        try {
            String ffmpegExe = getFfmpegPath();
    ...
    private static boolean encodeMax1440(String inputPath, String outputPath, String outputSegmentPath,
            Resolution resolution, Bitrate bitrate, boolean isHasAudio) {
        try {
            String ffmpegExe = getFfmpegPath();
    ...
    private static boolean encodeMaxOriginal(String inputPath, String outputPath, String outputSegmentPath,
            Resolution resolution, Bitrate bitrate, boolean isHasAudio) {
        try {
            String ffmpegExe = getFfmpegPath();
    ...
    public static boolean encodeHLSWithMultipleVideoStreams(String inputPath, String outputDir) {
        try {
            ...
            if (resolution.height <= 720) {
                ...
                success = encodeMax720(inputPath, outputPath, outputSegmentPath, resolution, bitrateConfig, isHasAudio);
            } else if (resolution.height <= 1080) {
                System.out.println("Encoding mode: Max 1080p");
                success = encodeMax1080(inputPath, outputPath, outputSegmentPath, resolution, bitrateConfig, isHasAudio);
            } else if (resolution.height <= 1440) {
                System.out.println("Encoding mode: Max 1440p");
                success = encodeMax1440(inputPath, outputPath, outputSegmentPath, resolution, bitrateConfig, isHasAudio);
            } else {
                System.out.println("Encoding mode: Original quality");
                success = encodeMaxOriginal(inputPath, outputPath, outputSegmentPath, resolution, bitrateConfig, isHasAudio);
            }
    ...
    public static boolean generateThumbnailWithFfmpeg(String inputPath, String outputPath) {
        try {
            ...
            ProcessBuilder pb = new ProcessBuilder(ffmpegExe, "-y", "-i", inputPath, "-ss", "00:00:01.000", 
                    "-vframes", "1", "-vf", "scale=320:-1", "-q:v", "2", outputPath);
