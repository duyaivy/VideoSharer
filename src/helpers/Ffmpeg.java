...
public class Ffmpeg {
	...
	public static boolean checkVideoHasAudio(String filePath) {
		try {
			...
			ProcessBuilder pb = new ProcessBuilder(ffprobeExe, "-v", "error", "-select_streams", "a:0", "-show_entries",
					"stream=codec_type", "-of", "default=nw=1:nk=1", filePath);
	...
	public static int getBitrate(String filePath) {
		try {
			...
			ProcessBuilder pb = new ProcessBuilder(ffprobeExe, "-v", "error", "-select_streams", "v:0", "-show_entries",
					"stream=bit_rate", "-of", "default=nw=1:nk=1", filePath);
	...
	private static boolean encodeMax720(String inputPath, String outputPath, String outputSegmentPath,
		...
		try {
			String ffmpegExe = PathHelper.getFfmpegPath();
	...
	public static boolean encodeHLSWithMultipleVideoStreams(String inputPath, String outputDir) {
		try {
			...
			if (resolution.height <= 720) {
				...
				success = encodeMax1080(inputPath, outputPath, outputSegmentPath, resolution, bitrateConfig,
						isHasAudio);
			} else if (resolution.height <= 1440) {
				System.out.println("Encoding mode: Max 1440p");
				success = encodeMax1440(inputPath, outputPath, outputSegmentPath, resolution, bitrateConfig,
						isHasAudio);
			} else {
				System.out.println("Encoding mode: Original quality");
				success = encodeMaxOriginal(inputPath, outputPath, outputSegmentPath, resolution, bitrateConfig,
						isHasAudio);
			}
	...
	public static boolean generateThumbnailWithFfmpeg(String inputPath, String outputPath) {
		try {
			...
			ProcessBuilder pb = new ProcessBuilder(ffmpegExe, "-y", "-i", inputPath, "-ss", "00:00:01.000", "-vframes",
					"1", "-vf", "scale=320:-1", "-q:v", "2", outputPath);
